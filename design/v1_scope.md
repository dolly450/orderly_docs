...
Στόχος δεν είναι να αποδείξουμε όλο το product vision. Στόχος είναι να αποδείξουμε ότι το QR ordering μειώνει την ουρά, αυξάνει απόδοση / ταχύτητα εξυπηρέτησης (throughput)/τζίρο και κάνει το staff ροή εργασίας (workflow) πιο γρήγορο με μετρήσιμα δεδομένα.

Το v1 μήνυμα πρέπει να μείνει καθαρό: **Scan -> Order -> Done**. Οτιδήποτε μπερδεύει αυτή τη ροή μπαίνει σε μελλοντικό στόχο (stretch goal) ή επόμενο tier.

Μετά το meeting με τον Σάββα Γεωργίου, υπάρχει σημαντική διευκρίνιση: το payment δεν εξετάζεται μόνο ως monetization ή λειτουργία ολοκλήρωσης αγοράς (checkout feature), αλλά ως **λειτουργικό σήμα (operational signal)** για commitment, prioritization και table turnover. Στο τρέχον MVP αυτό καλύπτεται από την επιλογή στο καλάθι: **πληρώνω τώρα** ή **μπαίνει στον λογαριασμό/ανοιχτό λογαριασμό (tab) μου**. Αν ο πελάτης επιλέξει πληρωμή τώρα, δηλώνει **μετρητά** ή **κάρτα**, ώστε να το διαχειριστεί το staff. Δεν χτίζουμε ακόμα βαρύ payment product.
...
5. Payment intent στο καλάθι: πληρώνω τώρα με μετρητά/κάρτα ή μπαίνει στον λογαριασμό/ανοιχτό λογαριασμό (tab)
...
9. Basic analytics για QR scans, cart additions, order submits, εγκαταλείψεις (drop-offs), χρόνο ολοκλήρωσης παραγγελίας και χρόνο εξυπηρέτησης. → [[Product Design]]
...
```mermaid
flowchart TD
    A[Πελάτης σκανάρει QR] --> B[Προβολή Web Μενού]
    B --> C[Προσθήκη στο Καλάθι]
    C --> D{Επιλογή Πληρωμής}
    D -->|Μετρητά/Κάρτα τώρα| E[Αποστολή Παραγγελίας]
    D -->|Στον λογαριασμό (Tab)| E
    E --> F[Ειδοποίηση KDS / Staff]
    F --> G[Ετοιμασία Παραγγελίας]
    G --> H[Παράδοση & Status Update]
```

## Εκτός v1 / Να μην μπερδέψουν το αρχικό pitch

- Full payments/commission product και πολύπλοκα payment integrations
- Room charge για ξενοδοχεία, ως μελλοντική επέκταση του ανοιχτό λογαριασμό (tab) flow
